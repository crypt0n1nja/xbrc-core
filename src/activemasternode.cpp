...
//
// Bootup the masternode, look for a 2500 XBRC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Bitrewards.conf */
...
{
    CBitrewardscoinAddress address(collateralAddress);
