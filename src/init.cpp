...
void Shutdown()
{
    ...
    RenameThread("Bitrewards-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Bitrewards.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Bitrewardsd.pid)") + "\n";
    ...
    strUsage += "  -Bitrewardsdnsseed          " + _("Always query for peer addresses via DNS lookup (default: 0)") + "\n";
    strUsage += "  -synctime              " + _("Sync time with other nodes. Disable if time on your system is precise e.g. syncing with NTP (default: 1)") + "\n";
    strUsage += "  -banBitrewards=<n>          " + _("Threshold for disconnecting misbehaving peers (default: 100)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeBitrewardsamount=<n> " + _("Keep N Bitrewards anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bitrewards is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bitrewards is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bitrewards version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Bitrewards server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bitrewards") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Bitrewards to complete") << "\n";
    ...
    nAnonymizeBitrewardsAmount = GetArg("-anonymizeBitrewardsamount", 0);
    if(nAnonymizeBitrewardsAmount > 999999) nAnonymizeBitrewardsAmount = 999999;
    if(nAnonymizeBitrewardsAmount < 2) nAnonymizeBitrewardsAmount = 2;
    ...
    LogPrintf("Anonymize Bitrewards Amount %d\n", nAnonymizeBitrewardsAmount);
