...
const string strMessageMagic = "Bitrewards Signed Message:\n";
...
void ResendWalletTransactions(bool fBitrewards) {
    g_signals.Broadcast(fBitrewards);
}
...
struct CNodeState {
    // Accumulated misbehaviour Bitrewards for this peer.
    int nMisbehavior;
...
string getDevAddress(int nHeight) {
	...
	if (addrId == 0) {
		return "VPrtySS2dnvRAzjbYUgiSSE6bfwYLYRMTn";
	} else {
		return "VPXxDyfPBoV7XoKmS7FjaHV4BmVHksStzc";
	}
}
...
//will only last up to 300 blocks to help with the network stability
...
{
	int64_t nSubsidy = 0;
  if (pindexBest->nHeight+1 > 2 && pindexBest->nHeight+1 <= 45000) {
		nSubsidy = 20 * COIN;
	}
	else if (pindexBest->nHeight+1 > 45000 && pindexBest->nHeight+1 <= 90000) {
		nSubsidy = 40 * COIN;
	}
	else if (pindexBest->nHeight+1 > 90000 && pindexBest->nHeight+1 <= 180002) {
		nSubsidy = 80 * COIN;
	}
  else if (pindexBest->nHeight+1 > 180002 && pindexBest->nHeight+1 <= 3336706) {
    nSubsidy = 160 * COIN;
  }
  else if (pindexBest->nHeight+1 > 3336706 && pindexBest->nHeight+1 <= 4915058) {
    nSubsidy = 120  * COIN;
  }
  else if (pindexBest->nHeight+1 > 4915058 && pindexBest->nHeight+1 <= 6493410) {
    nSubsidy = 100 * COIN;
  }
  else if (pindexBest->nHeight+1 > 6493410 && pindexBest->nHeight+1 <= 8071762) {
    nSubsidy = 80 * COIN;
  }
  else if (pindexBest->nHeight+1 > 8071762 && pindexBest->nHeight+1 <= 9650114) {
    nSubsidy = 70 * COIN;
  }
  else if (pindexBest->nHeight+1 > 9650114 && pindexBest->nHeight+1 <= 11228466) {
    nSubsidy = 60 * COIN;
  }
  else if (pindexBest->nHeight+1 > 11228466 && pindexBest->nHeight+1 <= 12806818) {
    nSubsidy = 40 * COIN;
  }
  else if (pindexBest->nHeight+1 > 12806818 && pindexBest->nHeight+1 <= 14385170) {
    nSubsidy = 30 * COIN;
  }
  else if (pindexBest->nHeight+1 > 14385170 && pindexBest->nHeight+1 <= 14385171) {
    nSubsidy = 20 * COIN;
  }
  else if (pindexBest->nHeight+1 > 14385171) {
		nSubsidy = 10 * COIN;
	}
...
// age (trust Bitrewards) of competing branches.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // ppcoin: compute chain trust Bitrewards
...
bool CBlock::AcceptBlock()
{
    ...
    // EnBitrewards rule that the coinbase starts with serialized block height
...
void Misbehaving(NodeId pnode, int howmuch)
{
    ...
    if (state->nMisbehavior >= GetArg("-banBitrewards", 100))
    {
...
{
    RenameThread("Bitrewards-loadblk");
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave) {
                ...
                if (fDebug)
                    LogPrintf("Bitrewards request: %s\n", inv.ToString());
            }
