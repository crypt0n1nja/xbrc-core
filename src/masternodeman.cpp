...
CMasternode* CMasternodeMan::GetCurrentMasterNode(int mod, int64_t nBlockHeight, int minProtocol)
{
    ...
    BOOST_FOREACH(CMasternode& mn, vMasternodes) {
        ...
        // calculate the Bitrewards for each masternode
        uint256 n = mn.CalculateBitrewards(mod, nBlockHeight);
        unsigned int n2 = 0;
...
{
    std::vector<pair<unsigned int, CTxIn> > vecMasternodeBitrewardss;
    ...
    BOOST_FOREACH(CMasternode& mn, vMasternodes) {
        ...
        uint256 n = mn.CalculateBitrewards(1, nBlockHeight);
        unsigned int n2 = 0;
        memcpy(&n2, &n, sizeof(n2));

        vecMasternodeBitrewardss.push_back(make_pair(n2, mn.vin));
    }

    sort(vecMasternodeBitrewardss.rbegin(), vecMasternodeBitrewardss.rend(), CompareValueOnly());

    int rank = 0;
    BOOST_FOREACH (PAIRTYPE(unsigned int, CTxIn)& s, vecMasternodeBitrewardss){
        rank++;
        if(s.second == vin) {
            return rank;
        }
...
{
    std::vector<pair<unsigned int, CMasternode> > vecMasternodeBitrewardss;
    ...
    BOOST_FOREACH(CMasternode& mn, vMasternodes) {
        ...
        vecMasternodeBitrewardss.push_back(make_pair(n2, mn));
    }

    sort(vecMasternodeBitrewardss.rbegin(), vecMasternodeBitrewardss.rend(), CompareValueOnlyMN());

    int rank = 0;
    BOOST_FOREACH (PAIRTYPE(unsigned int, CMasternode)& s, vecMasternodeBitrewardss){
        rank++;
