...
bool GetLocal(CService& addr, const CNetAddr *paddrPeer)
{
    ...
    int nBestBitrewards = -1;
    int nBestReachability = -1;
    {
        LOCK(cs_mapLocalHost);
        for (map<CNetAddr, LocalServiceInfo>::iterator it = mapLocalHost.begin(); it != mapLocalHost.end(); it++)
        {
            int nBitrewards = (*it).second.nBitrewards;
            int nReachability = (*it).first.GetReachabilityFrom(paddrPeer);
            if (nReachability > nBestReachability || (nReachability == nBestReachability && nBitrewards > nBestBitrewards))
            {
                addr = CService((*it).first, (*it).second.nPort);
                nBestReachability = nReachability;
                nBestBitrewards = nBitrewards;
            }
        }
    }
    return nBestBitrewards >= 0;
}
...
int GetnBitrewards(const CService& addr)
{
    LOCK(cs_mapLocalHost);
    if (mapLocalHost.count(addr) == LOCAL_NONE)
        return 0;
    return mapLocalHost[addr].nBitrewards;
}
...
// used when Bitrewardss of local addresses may have changed
...
bool AddLocal(const CService& addr, int nBitrewards)
{
    if (!addr.IsRoutable())
        return false;

    if (!fDiscover && nBitrewards < LOCAL_MANUAL)
        return false;

    if (IsLimited(addr))
        return false;

    LogPrintf("AddLocal(%s,%i)\n", addr.ToString(), nBitrewards);

    {
        LOCK(cs_mapLocalHost);
        bool fAlready = mapLocalHost.count(addr) > 0;
        LocalServiceInfo &info = mapLocalHost[addr];
        if (!fAlready || nBitrewards >= info.nBitrewards) {
            info.nBitrewards = nBitrewards + (fAlready ? 1 : 0);
...
bool AddLocal(const CNetAddr &addr, int nBitrewards)
{
    return AddLocal(CService(addr, GetListenPort()), nBitrewards);
}
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Bitrewards " + FormatFullVersion();
...
void ThreadDNSAddressSeed()
{
    ...
    if ((addrman.size() > 0) &&
        (!GetBoolArg("-Bitrewardsdnsseed", true))) {
        MilliSleep(11 * 1000);
...
// most recently
static int64_t NodeSyncBitrewards(const CNode *pnode) {
    return pnode->nLastRecv;
}

void static StartSync(const vector<CNode*> &vNodes) {
    CNode *pnodeNewSync = NULL;
    int64_t nBestBitrewards = 0;
    ...
    BOOST_FOREACH(CNode* pnode, vNodes) {
        ...
        if (!pnode->fClient && !pnode->fOneShot &&
            ...
            // if ok, compare node's Bitrewards with the best so far
            int64_t nBitrewards = NodeSyncBitrewards(pnode);
            if (pnodeNewSync == NULL || nBitrewards > nBestBitrewards) {
                pnodeNewSync = pnode;
                nBestBitrewards = nBitrewards;
            }
        }
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Bitrewards is probably already running."), addrBind.ToString());
        else
