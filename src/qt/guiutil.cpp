...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Bitrewards"))
        return false;
...
{
    // Convert Bitrewards:// to Bitrewards:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Bitrewards://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Bitrewards:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitrewards.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Bitrewards.lnk
...
{
    return GetAutostartDir() / "Bitrewards.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Bitrewards.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Bitrewards\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Bitrewards app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Bitrewards app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bitrewards-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Bitrewards-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Bitrewards-Qt"));
