...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBitrewardsAmount"))
        settings.setValue("nAnonymizeBitrewardsAmount", 1000);
    nAnonymizeBitrewardsAmount = settings.value("nAnonymizeBitrewardsAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBitrewardsAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBitrewardsAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBitrewardsAmount:
            return QVariant(nAnonymizeBitrewardsAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBitrewardsAmount:
            nAnonymizeBitrewardsAmount = value.toInt();
            settings.setValue("nAnonymizeBitrewardsAmount", nAnonymizeBitrewardsAmount);
            emit AnonymizeBitrewardsAmountChanged(nAnonymizeBitrewardsAmount);
            break;
        default:
            break;
        }
