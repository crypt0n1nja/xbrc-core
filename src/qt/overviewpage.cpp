...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBitrewardsAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBitrewardsAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBitrewardsAmount = strAnonymizeBitrewardsAmount.remove(strAnonymizeBitrewardsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitrewardsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBitrewardsAmount*COIN) nMaxToAnonymize = nAnonymizeBitrewardsAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBitrewardsAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBitrewardsAmount));
        strAnonymizeBitrewardsAmount = strAnonymizeBitrewardsAmount.remove(strAnonymizeBitrewardsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitrewardsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeBitrewardsAmount == 0){
