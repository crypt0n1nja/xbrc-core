...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <Bitrewardsaddress> <amount>\n"
            "Bitrewardsaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <Bitrewardsaddress> <amount>\n"
            "Bitrewardsaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        (strCommand != "count" && strCommand != "current" && strCommand != "debug" && strCommand != "genkey" && strCommand != "enBitrewards" && strCommand != "list" && strCommand != "list-conf"
        ...
        throw runtime_error(
                ...
                "  enBitrewards      - EnBitrewards masternode payments\n"
                "  list         - Print list of all known masternodes (see masternodelist for more info)\n"
                "  list-conf    - Print masternode.conf in JSON format\n"
                "  outputs      - Print masternode compatible outputs\n"
                "  start        - Start masternode configured in Bitrewards.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in Bitrewards.conf\n"
                ...
                "  vote-many    - Vote on a Bitrewards initiative\n"
                "  vote         - Vote on a Bitrewards initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CBitrewardscoinSecret(secret).ToString();
    }
    ...
    if(strCommand == "enBitrewards")
    {
        return (uint64_t)enBitrewardsMasternodePaymentsTime;
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Bitrewards initiative (can be additionally filtered, partial match)\n"
