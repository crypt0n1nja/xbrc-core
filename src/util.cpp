...
int nAnonymizeBitrewardsAmount = 1000;
int nLiquidityProvider = 0;
/** Spork enBitrewardsment enabled time */
int64_t enBitrewardsMasternodePaymentsTime = 4085657524;
...
#else
    const char* pszModule = "Bitrewards";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitrewards
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitrewards
    // Mac: ~/Library/Application Support/Bitrewards
    // Unix: ~/.Bitrewards
#ifdef WIN32
    // Windows
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Bitrewards.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Bitrewardsd.pid"));
