...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CBitrewardscoinAddress(redeemScript.GetID()).ToString();
        ...
        && UnlockStealthAddresses(vMasterKey))
            {
                ...
                if (pMasterKey.second.nDeriveIterations < 2500)
                    pMasterKey.second.nDeriveIterations = 2500;
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
    ...
    if (kMasterKey.nDeriveIterations < 2500)
...
void CWallet::SyncTransaction(const CTransaction& tx, const CBlock* pblock, bool fConnect)
{
    ...
    // available of the outputs it spends. So Bitrewards those to be
    // recomputed, also:
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
	                LogPrintf("ReacceptWalletTransactions found spent coin %s XBRC %s\n", FormatMoney(wtx.GetCredit(ISMINE_ALL)), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
void CWallet::ResendWalletTransactions(bool fBitrewards)
{
    if (!fBitrewards)
    {
    ...
    CTxDB txdb("r");
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (fBitrewards || nTimeBestReceived - (int64_t)wtx.nTimeReceived > 5 * 60)
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
	                    ...
	                    strFailReason = _(" Unable to locate enough Darksend non-denominated funds for this transaction that are not equal 1000 XBRC.");
                    } else {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-Bitrewards-address
                    CScript scriptChange;
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        CBitrewardscoinAddress addr(ckid);
        ...
        if (fDebug)
        {
            ...
            CBitrewardscoinAddress coinAddress(keyID);
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    CBitrewardscoinAddress addrTo(ckidTo);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if(hasPayment && payeerewardpercent == 0){
        ...
        CBitrewardscoinAddress address2(address1);
    ...
    if(hasPayment && payeerewardpercent > 0 && payeerewardpercent < 100){
        ...
        CBitrewardscoinAddress address4(address3);
    ...
    int64_t devfee = 0;
    int64_t masternodePayment = nReward * 3/4;  //75% goes to the MN

    // Add the dev fees payment (block 1201 to be change when hardfork decided and code ready)
    if (pindexPrev->nHeight+1 > 1201) {
        masternodePayment = nReward * 0.75;
        devfee = nReward * 0.00;  //no need for dev fee
        ...
        CBitrewardscoinAddress devRewardAddress(getDevAddress(pindexPrev->nHeight+1));
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CBitrewardscoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CBitrewardscoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CBitrewardscoinAddress(address).ToString());
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
	            LogPrintf("FixSpentCoins found lost coin %s XBRC %s[%d], %s\n",
            ...
            {
	            LogPrintf("FixSpentCoins found spent coin %s XBRC %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
